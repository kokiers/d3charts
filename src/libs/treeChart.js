  ...
  constructor({el,width,height}) {
    ...
    initTree() {
      ...
      list = list.map(v=>{
        ...
        if (!sourceNode[sourceId]){
          sourceNode[sourceId] = {...nodes[v.source],targetId:sourceId}
        }
        if (!sourceNode[targetId]){
          sourceNode[targetId] = {...nodes[v.target],targetId:targetId}
        }
        v.sourceId = sourceId
        v.targetId = targetId
        return v
    ...
    drawNodeText() {
      ...
      this.g.selectAll('.node').append('text')
        ...
        .text((d) => {
          let link = d.data.data.link,title = ''
          if (link){
    ...
    clickDetail(d){
      let { source, target, targetId, level} = d.data.data.link
      if (level < 0){
        return
      }
